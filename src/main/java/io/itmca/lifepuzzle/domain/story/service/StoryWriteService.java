...
public class StoryWriteService {
    ...
    private void saveFiles(String folderPath, List<String> filePaths, List<MultipartFile> files) throws IOException {
        var len = filePaths.size();

        for(var i = 0; i < len ; i++){
            var targetMultiPartFile = files.get(i);
            var tempFilePath = String.format(FileConstant.TEMP_FOLDER_PATH + File.separator + filePaths.get(i));
